...
func NewFilePartDao(impl dao.IFilePartDao) dao.IFilePartDao {
	cc := lru.NewLRU[filePartPair, *entity.FilePartInfoItem](
		defaultMaxFilePartCacheSize,
		nil,
		defaultFilePartCacheExpireTime,
	)
	return &filePartDao{
		IFilePartDao: impl,
...
func (f *filePartDao) CreateFilePart(
	ctx context.Context,
	req *entity.CreateFilePartRequest,
) (*entity.CreateFilePartResponse, error) {
	// filepartid 能被覆盖, 所以创建后需要清理缓存
	defer func() {
		_ = f.cache.Del(ctx, filePartPair{FileId: req.FileId, PartId: req.FilePartId})
	}()
	response, err := f.IFilePartDao.CreateFilePart(ctx, req)
	if err != nil {
		return nil, fmt.Errorf("create cached file part: %w", err)
	}
	return response, nil
}

func (f *filePartDao) GetFilePartInfo(
	ctx context.Context,
	...
	m, err := cacheapi.LoadMany(
		...
		f.cache,
		ks,
		func(ctx context.Context, miss []filePartPair) (map[filePartPair]*entity.FilePartInfoItem, error) {
			...
			if err != nil {
				return nil, fmt.Errorf("read uncached file parts: %w", err)
			}
	...
	if err != nil {
		return nil, fmt.Errorf("load file part cache: %w", err)
	}
	...
	for _, k := range ks {
		...
		if !ok {
			logutil.GetLogger(ctx).Error(
				"cache key not found",
				zap.Uint64("file_id", req.FileId),
				zap.Int32("file_part_id", k.PartId),
			)
			continue
		}
