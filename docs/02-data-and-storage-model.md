...
`(file_id, file_part_id)` 唯一，`file_part_id` 从零开始。
...
### 2.5 `tg_file_part_delete_state_tab`
...
Part 与 Delete State 在同一个事务中插入；如果事务失败，FileManager立即尝试补偿删除刚
上传的 message。新建 Mapping 前必须确认该 File 的全部状态为 `live`。
...
S3 PUT、CopyObject、DeleteObject 和覆盖在 Directory 事务内完成：
